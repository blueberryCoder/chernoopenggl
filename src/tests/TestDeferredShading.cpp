...
namespace test {
    TestDeferredShading::TestDeferredShading() {
        ...
        m_LightingShader = std::make_shared<Shader>(
            FileUtil::shared().GetPath("./shaders/test_deferred_shading.shader"));
    ...
    void TestDeferredShading::OnUpdate(float deltaTime) {
        ...
        for (int i = 0; i < kLightCount; ++i) {
            m_LightingShader->SetUniformVec3f(
                "lights[" + std::to_string(i) + "].Position", m_LightPositions[i]);
            m_LightingShader->SetUniformVec3f(
                "lights[" + std::to_string(i) + "].Color", m_LightColors[i]);
            m_LightingShader->SetUniform1f(
                "lights[" + std::to_string(i) + "].Linear", m_LightLinear);
            m_LightingShader->SetUniform1f(
                "lights[" + std::to_string(i) + "].Quadratic", m_LightQuadratic);
        }
    }
    ...
    void TestDeferredShading::OnImGuiRender() {
        ImGui::Text("Deferred shading (GBuffer -> lighting)");
        ImGui::Text("Models: %zu", m_ObjectPositions.size());
        ImGui::Text("Lights: %zu", m_LightPositions.size());
    }

    void TestDeferredShading::ProcessInputEvent(GLFWwindow *window, float deltaTime) {
        m_Camera->ProcessInputEvent(window, deltaTime);
    }

    void TestDeferredShading::ProcessCursorPosCallback(GLFWwindow *window, double xpos, double ypos) {
        m_Camera->ProcessCursorPosCallback(window, xpos, ypos);
    }

    void TestDeferredShading::ProcessMouseScroll(GLFWwindow *window, double yoffset) {
        m_Camera->ProcessMouseScroll(yoffset);
    }

    void TestDeferredShading::CreateGBuffer(int width, int height) {
